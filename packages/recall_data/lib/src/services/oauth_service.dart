...
import 'package:common/extensions/string_extension.dart';
...
class OAuthService {
  ...
  Future<OAuthData> authenticateWithGoogle(String code) async {
    try {
      ...
      return OAuthData(
        ...
        firstName: userData['given_name'] as String? ?? firstName,
        lastName: userData['family_name'] as String? ?? lastName,
  ...
  Future<OAuthData> authenticateWithGitHub(String code) async {
    try {
      ...
      if (tokenResponse.statusCode != 200) {
        throw Exception('Failed to get GitHub access token');
      }

      final tokenData = jsonDecode(tokenResponse.body) as Map<String, dynamic>;

      if (tokenData.containsKey('error')) {
        throw Exception('GitHub token exchange failed');
      }
      ...
      if (userInfoResponse.statusCode != 200) {
        throw Exception('Failed to get GitHub user info');
      }
      ...
      throw Exception('GitHub authentication failed');
    }
  }
